...
<nav x-data="{ open: false }" class="bg-gray-800 border-b border-gray-700 text-white">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            ...
            <!-- Settings Dropdown (Breeze) -->
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button
                        class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md
                                text-gray-200 bg-gray-800 hover:text-white hover:bg-gray-700 focus:outline-none transition">
                        <div>{{ Auth::user()->name }}</div>
                        <svg
                            class="ms-1 h-4 w-4 text-gray-200 transition-transform duration-200"
    ...
    <div :class="{ 'block': open, 'hidden': ! open }" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @can('admin')
                <x-responsive-nav-link :href="route('admin.temas.index')" :active="request()->routeIs('admin.temas.*')">
                    {{ __('Temas') }}
                </x-responsive-nav-link>
            @endcan
        </div>
